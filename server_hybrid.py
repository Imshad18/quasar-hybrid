...
def is_harmful_rules(conversation):
...
def is_harmful_llm(conversation):
...
Respond ONLY with: {"harmful": true} or {"harmful": false}"""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            parsed = eval(text)
            return parsed.get("harmful", False)
        else:
            return False
...
def classify(conversation: Conversation):
    ...
    if is_harmful_rules(conversation):
        return {
            "violation": True,
            "confidence": 1.0
        }
    ...
    harmful = is_harmful_llm(conversation)
